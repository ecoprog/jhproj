...
export class ComponentService {
    ...
    /* Convert a returned JSON object to Points entity */
    private convertItemFromServer(json: any): Points {
        const entity: Points = Object.assign(new Points(), json);
        entity.date = this.dateUtils.convertLocalDateFromServer(json.date);
        return entity;
    }

    /* Convert a Points entity to a JSON which can be sent to the server */
    private convert(points: Points): Points {
        const copy: Points = Object.assign({}, points);
        copy.date = this.dateUtils.convertLocalDateToServer(points.date);
        return copy;
    }
}
